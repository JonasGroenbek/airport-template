...
public class PassengerProducer {
    ...
    public PassengerProducer(List<Plane> planes, PriorityQueue<Passenger> queue) {
        this.planes = planes;
        this.queue = queue;
        lastDepartureTime = planes.get(planes.size() - 1).getDepartureTime();
    }

    //if still processingTicks return
    //If now is after lastDepartureTime stop clock
    public void tick(Clock clock) {
        ...
        processingTicksLeft = randomizer.nextInt(100);
    }
